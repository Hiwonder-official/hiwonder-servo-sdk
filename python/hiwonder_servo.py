"""
hiwonder_servo.py — Python SDK for HX-30HM bus servo
...
class Reg:
    ...
    SPD_I          = 0x27   # 2 bytes
    TARGET_POS     = 0x2A   # 2 bytes
    MOVE_TIME      = 0x2C   # 2 bytes, ms (also PWM open-loop)
...
class HiwonderBus:
    ...
    def _recv(self, expected_params: int) -> tuple[int, int, list[int]]:
        ...
        # Minimum response: ID(1) + Len(1) + ERROR(1) + CheckSum(1) = 4 bytes after header
